...
```
```{r}
m7 <- lm(peso_1 ~ ac_vaca*sexo, data=ac_vaca_dat[-c(23), ] )
plot(m7)
...
#realice la prueba U de Mann Whitney
 wilcox.test (vacas_tv ~ vacas_no_tv, data = transm)
...
reg7 <- lm(ac ~ sexo, data = serocon_dat)
reg8 <- lm(ac ~ dias*sexo, data = serocon_dat)
reg9 <- lm(ac ~ dias+sexo, data =serocon_dat)
